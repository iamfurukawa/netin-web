...
export function ReactionManager() {
  ...
  return <section className="reactions-card" aria-labelledby="reactions-title">
    ...
    <form className="reaction-create-form" onSubmit={submitCreate}>
      <label>Imagem ou GIF<input accept="image/jpeg,image/gif" onChange={(event) => setAsset(event.target.files?.[0] ?? null)} type="file" required /></label>
      <label>Nome<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex.: Festa" maxLength={32} required /></label>
      <label>Ordem<input value={displayOrder} onChange={(event) => setDisplayOrder(event.target.value)} type="number" min="0" max="10000" required /></label>
    ...
    <div className="reaction-admin-list">
      {reactions.data?.reactions.map((reaction) => <form key={reaction.id} className="reaction-admin-row" onSubmit={(event) => submitUpdate(event, reaction)}>
        <div className="reaction-admin-row__asset">{reaction.assetKind ? <img src={apiUrl(reaction.assetPath)} alt="" /> : <span className="reaction-admin-placeholder">Sem imagem</span>}</div>
        <div className="reaction-admin-row__fields"><label>Nome<input defaultValue={reaction.name} name="name" maxLength={32} required /></label><label>Ordem<input defaultValue={reaction.displayOrder} name="displayOrder" type="number" min="0" max="10000" required /></label></div>
        <label className="reaction-admin-row__file">Trocar imagem ou GIF<input accept="image/jpeg,image/gif" name="asset" type="file" /></label>
        <div className="reaction-admin-row__actions"><label className="toggle-row"><input defaultChecked={reaction.isActive} name="isActive" type="checkbox" /><span>Ativa</span></label><button className="button--secondary" disabled={update.isPending} type="submit">Salvar</button></div>
      </form>)}
    </div>
