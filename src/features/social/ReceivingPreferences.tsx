...
export function ReceivingPreferences() {
  ...
  return <section className="social-preferences" aria-labelledby="social-preferences-title">
    ...
    <label className="toggle-row"><input type="checkbox" checked={preferences.data?.muted ?? false} disabled={preferences.isPending || change.isPending} onChange={(event) => change.mutate(event.target.checked)} /><span>Silenciar interações recebidas</span></label>
    <p className="muted">Enquanto estiver silenciado, novas reações, mensagens e cutucadas não serão entregues aos seus Netins.</p>
